...
export default function MasterPlantPage() {
  ...
  const [f, setF] = useState({ id_plant: "", nama_plant: "" });
  ...
  const filtered = rows.filter(
    (r) => !keyword || `${r.id_plant} ${r.nama_plant}`.toLowerCase().includes(keyword.toLowerCase())
  );
  ...
  async function simpan(e: React.FormEvent) {
    ...
    try {
      if (editId) {
        await api(`/plant/${editId}`, { method: "PUT", body: JSON.stringify({ nama_plant: f.nama_plant }) });
        setMsg("Plant diperbarui.");
      } else {
        await api("/plant", { method: "POST", body: JSON.stringify({ id_plant: f.id_plant, nama_plant: f.nama_plant }) });
        setMsg("Plant disimpan.");
      }
      setShowForm(false);
      setEditId(null);
      setF({ id_plant: "", nama_plant: "" });
      load();
    } catch (e) {
  ...
  async function hapus(id: string) {
    const ok = await window.confirm(`Hapus plant ${id}?`);
    if (!ok) return;
    try {
      await api(`/plant/${id}`, { method: "DELETE" });
      load();
    } catch (e) {
    ...
    <div className="flex flex-col gap-[7px]">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-[#e9edf5] p-2.5">
          <div className="relative min-w-[200px] flex-1">
            ...
            <input
              ...
              placeholder="Cari ID atau nama plant"
          ...
          <table className="w-full min-w-[460px] border-collapse text-left">
            <thead>
              <tr className="bg-[#fbfcff]">
                <th className="w-[130px] px-3 py-2 text-[10px] font-black uppercase tracking-wide text-[#6b7280]">ID</th>
                <th className="px-3 py-2 text-[10px] font-black uppercase tracking-wide text-[#6b7280]">Nama Plant</th>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-3 py-6 text-center text-[11px] font-bold text-[#6b7280]">
                    Tidak ada data plant.
                  </td>
                </tr>
              )}
              {filtered.map((r, i) => (
                <tr key={r.id_plant} className={i % 2 ? "bg-[#fbfcff]" : "bg-white"}>
                  <td className="px-3 py-2 text-[12px] font-black text-[#172033]">{r.id_plant}</td>
                  <td className="px-3 py-2 text-[12px] font-bold text-[#172033]">{r.nama_plant}</td>
                  <td className="px-3 py-2">
                    <div className="flex justify-end gap-1.5">
                      <button
                        onClick={() => {
                          setEditId(r.id_plant);
                          setF({ id_plant: r.id_plant, nama_plant: r.nama_plant });
                          setShowForm(true);
                        }}
      ...
      {showForm && (
        <div className="fixed inset-0 z-[1060] flex items-center justify-center bg-[rgba(15,23,42,0.45)] p-4" onClick={() => setShowForm(false)}>
          <form onSubmit={simpan} className="w-full max-w-[400px] rounded-2xl bg-white p-4 shadow-[0_20px_60px_rgba(0,0,0,0.3)]" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 text-[15px] font-black tracking-tight text-[#191970]">{editId ? `Edit Plant ${editId}` : "Tambah Plant"}</div>
            <div className="flex flex-col gap-2.5">
              {!editId && (
                <div>
                  <label className={labelCls}>ID Plant</label>
                  <input className={inputCls} value={f.id_plant} onChange={(e) => setF({ ...f, id_plant: e.target.value })} required />
                </div>
              )}
              <div>
                <label className={labelCls}>Nama Plant</label>
                <input className={inputCls} value={f.nama_plant} onChange={(e) => setF({ ...f, nama_plant: e.target.value })} required />
              </div>
            </div>
